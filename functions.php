...
require_once('library/winter.php');            // core functions (don't remove)
...
add_filter('admin_footer_text', 'winter_custom_admin_footer');
function winter_custom_admin_footer() {
 ...
 */
function winter_filter_title( $title, $sep ) {
	global $paged, $page;
	...
	if ( $paged >= 2 || $page >= 2 ) {
		$title = "$title $sep " . sprintf( __( 'Page %s', 'winterlude-theme' ), max( $paged, $page ) );
	}

	return $title;
}
add_filter( 'wp_title', 'winter_filter_title', 10, 2 );
...
function winter_register_sidebars() {
...
// Comment Layout
function winter_comments($comment, $args, $depth) {
		...
		<article id="comment-<?php comment_ID(); ?>" class="clearfix">
			<div class="comment-author vcard row-fluid clearfix">
				...
				<div class="span9 comment-text">
					...
					<?php edit_comment_link(__('Edit','winterlude-theme'),'<span class="edit-comment btn btn-small btn-info"><i class="icon-white icon-pencil"></i>','</span>') ?>
                    
                    <?php if ($comment->comment_approved == '0') : ?>
       					<div class="alert-message success">
          				<p><?php _e('Your comment is awaiting moderation.','winterlude-theme') ?></p>
          				</div>
					<?php endif; ?>
...
// Search Form
function winter_wpsearch( $form ) {
  $form = '<form role="search" method="get" id="searchform" action="' . home_url( '/' ) . '" >
  <label class="screen-reader-text" for="s">' . __('Search for:', 'winterlude-theme') . '</label>
  <input type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="Search the Site..." />
  <input type="submit" id="searchsubmit" value="'. esc_attr__('Search','winterlude-theme') .'" />
  </form>';
  return $form;
} // don't remove this bracket!
...
function custom_password_form() {
	...
	' . '<p>' . __( "This post is password protected. To view it please enter your password below:" ,'winterlude-theme') . '</p>' . '
	<label for="' . $label . '">' . __( "Password:" ,'winterlude-theme') . ' </label><div class="input-append"><input name="post_password" id="' . $label . '" type="password" size="20" /><input type="submit" name="Submit" class="btn btn-primary" value="' . esc_attr__( "Submit",'winterlude-theme' ) . '" /></div>
	</form></div>
	';
	return $o;
}
...
function your_custom_menu_item ( $items, $args ) {
	...
	if (! is_front_page()){
		...
		if ( ( $locations = get_nav_menu_locations() ) && isset( $locations[ $menu_name ] ) ) {
			$myMenu='<li id="menu-item-home"><a href="/">Home</a></li>'; // parent
			$newMenu = $myMenu;
			$newMenu .= $items;
			//$items .= $myMenu;
			return $newMenu;
...
if( !function_exists( "theme_js" ) ) {  
  function theme_js(){
    ...
    wp_register_script(  'modernizr', 
      get_template_directory_uri() . '/library/js/libs/modernizr-2.6.2.min.js', 
      array('jquery'), 
      '1.2' );
	  
	wp_register_script(  'vidz', 
      get_template_directory_uri() . '/library/js/jquery.fitvids.js', 
      array('jquery'), 
      '1.2' ); 
 	wp_register_script(  'snowflakes', 
      get_template_directory_uri() . '/library/js/libs/jquery.snow.min.1.0.js', 
      array('jquery'), 
      '1.2' );
	
    wp_enqueue_script('bootstrap');
    wp_enqueue_script('modernizr');
	wp_enqueue_script('vidz');
	wp_enqueue_script('snowflakes');
